...
def listing_view(request, id):
    try:
        ...
        messages.error(request, f'Invalid UID {id} was provided for listing product')
        return redirect('home')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if listing_form.is_valid and location_form.is_valid:
                listing_form.save()
                location_form.save()
                messages.info(request, f'Listing {listing.model} updated successfully!')
                return redirect('home')
            else:
                messages.error(
        ...
        print(f"[DEBUG] Edit view error: {e}")
        messages.error(
